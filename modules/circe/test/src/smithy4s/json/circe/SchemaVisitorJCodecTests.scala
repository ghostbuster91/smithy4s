...
import io.circe._
import smithy4s.json.circe.Json._
...
class SchemaVisitorJCodecTests() extends FunSuite {
  ...
  def readFromString[A: Codec](str: String): A = {
    io.circe.parser.decode[A](str) match {
      case Left(err) => throw err
      case Right(v)  => v
    }
  }

  def writeToString[A: Codec](a: A): String = {
    Codec[A].apply(a).toString
  }
  ...
  test("Lenient union") {
    ...
    implicit val codec: Codec[Either[Int, String]] =
  ...
  test("Lenient and regular unions have the same error messages") {
    ...
    def decodeCheck(codec: Codec[Either[String, String]]) =
      expect.same(
        Try(
